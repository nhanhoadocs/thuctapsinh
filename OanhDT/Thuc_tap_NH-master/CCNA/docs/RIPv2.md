...
![Topology](../images/CauhinhRIPv2/Topo.png)
 - Trước tiên chúng ta đặt địa chỉ ip cho từng PC trong topo 
    - IP PC0

    ![ipPC0](../images/CauhinhRIPv2/ipPC0.png)

    - IP PC1

    ![ipPC1](../images/CauhinhRIPv2/ipPC1.png)

    - IP PC2 

    ![ipPC2](../images/CauhinhRIPv2/ipPC2.png)
...
```
    ...
    R3(config-if)#ip add 168.16.10.254 255.255.0.0
    R3(config-if)no shutdown
    R3(config-if)#exit 
    R3(config)#interface se0/0/0
    R3(config-if)#ip add 168.16.20.2 255.255.0.0
    R3(config-if)no shutdown
    R3(config)#interface se0/0/0
    R3(config-if)#ip add 168.16.30.3 255.255.0.0
    R3(config-if)no shutdown
    R3(config-if)#exit 
...
![Ping](../images/CauhinhRIPv2/ping.png)
